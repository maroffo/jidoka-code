...
Installed production authority is currently blocked on standard macOS `/Applications`: the directory is normally `root:admin` mode `0775`, and an admin user can rename its children without authorization. The production resolver intentionally rejects that ancestor. A root-owned app bundle alone is not sufficient. Do not install for production, launch the normal app, or relax the ownership check until a reviewed non-renamable installation authority is selected.
