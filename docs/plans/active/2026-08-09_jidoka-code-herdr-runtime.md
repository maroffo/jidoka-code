...
- Autorizzati il 2026-08-11 dopo il merge PR #13: branch/worktree W8/W9 da exact `origin/main@6350acc454878b7fed2d84ea6802ef8ebf804b56`, identifier production, signing locale e disposable staging. Le Keychain Application/Installer effimere e la search list originale sono state ripulite; package install, ServiceManagement, credenziali applicative e canary restano ai checkpoint del piano principale.
...
- [ ] 2026-08-11: W8 sostituisce gli identifier probe con `com.maroffo.JidokaCode` e `com.maroffo.JidokaCode.Engine`; uno staging signed storico passa dopo rimozione checkout e preflight da cwd `/`, ma precede i successivi fix termination/S9 e deve essere ripetuto dal tree finale. S9 automatico è ristretto a package/topology statici: il launch production resta live-only dopo che una prima variante ha aggiornato il lock UI nella home reale e ha lasciato `unknown` un eventuale tentativo di contatto al socket default. Evidence: [`docs/evidence/w8-production-identifier-staging-report.md`](../../evidence/w8-production-identifier-staging-report.md).
...
Gli identifier production sono chiusi nel piano W8; lo staging signed precedente resta evidence storica ma deve essere ripetuto dal tree finale dopo i fix review. Restano final notarization del nuovo package, installazione, ServiceManagement, runtime accessibility, credential GitHub/provider e canary sulla sessione default. Ognuno richiede il checkpoint specifico già definito.
