...
- [ ] 2026-08-12: W8 usa `com.maroffo.JidokaCode` e `com.maroffo.JidokaCode.Engine`; il package notarizzato `94382d03…` è revocato dopo che CHECKPOINT C ha scritto il receipt ma PackageKit ha rilocato il bundle nel worktree anziché `/Applications`, con mode `0700`. Nessun launch production, job o default-session canary è avvenuto. Component policy non-relocatable e mode distributivi sono in fix; servono nuovo staging, notarization e CHECKPOINT C. Evidence: [`docs/evidence/w8-production-identifier-staging-report.md`](../../evidence/w8-production-identifier-staging-report.md).
...
Gli identifier production restano chiusi, ma lo staging/notarization del package `94382d03…` è revocato dal falsifier installato di relocation e mode. Restano fix packaging, nuovo staging/notarization, cleanup autorizzato del failure state, nuovo CHECKPOINT C, installazione, ServiceManagement, runtime accessibility, credential GitHub/provider e canary sulla sessione default. Ognuno richiede il checkpoint specifico già definito.
