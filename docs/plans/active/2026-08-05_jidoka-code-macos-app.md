...
- [ ] Ripetere package build dal tree finale in un disposable staging checkout creato per il test, registrare il target e rimuovere soltanto quel path dopo build. Il bundle deve poi avviarsi con cwd `/`; il normale implementation worktree non viene spostato o nascosto. Il merge product-bearing `c218e52…` è stato riprodotto, ma il successivo install autorizzato ha revocato package `94382d03…`: PackageKit lo ha rilocato sopra un bundle omonimo nel worktree e ha installato mode `0700`. La fix richiede una nuova reproduction, notarization e digest.
...
- [x] Su host non-macOS il gate deve dichiarare che la release macOS non è verificabile; nessuna release può basarsi soltanto su quel risultato.
...
- [ ] W8 packaging, installer, documentation and repository gates. Identifier production e gate non-macOS sono chiusi; l'install autorizzato ha riaperto component packaging, disposable staging, notarization, CHECKPOINT C, cleanup del failure state e verifica installed.
...
- 2026-08-11: dopo il merge PR #13, il project owner autorizza branch/worktree W8/W9 da exact `origin/main@6350acc454878b7fed2d84ea6802ef8ebf804b56`, identifier production e verifica locale. App, LaunchAgent e Mach service diventano `com.maroffo.JidokaCode` e `com.maroffo.JidokaCode.Engine`; il receipt resta `com.maroffo.JidokaCode.pkg`.
- 2026-08-11: disposable staging passa con checkout rimosso prima del preflight da cwd `/`. Un falsifier reale su Bash 3.2, signing hang e prompt Keychain porta a runner bounded, `productbuild` unsigned più `productsign --timestamp` e Keychain Application/Installer separati. Package staging signed ma non notarized `c73d85457ac2f8124de0b668a6bf0b515decfa306df8f40874fa6f389a4b21c3`; installazione assente. Evidence: `docs/evidence/w8-production-identifier-staging-report.md`.
- 2026-08-11: durante la closure review, una prima variante S9 preflight avvia la composition production con `HOME` sintetica, ma `FileManager.homeDirectoryForCurrentUser` risolve comunque la home reale e aggiorna `IPC/ui-instance.lock`. App build, temp path, receipt e job launchd sono poi assenti; un eventuale tentativo di contatto al socket Herdr default resta `unknown` per mancanza di evidence retained e non vale come canary W9. S9 preflight ora si ferma alle verifiche statiche; il launch production resta live-only e authorization-gated.
- 2026-08-11: il final review rileva che lo staging signed `c73d854…` precede i fix al durable termination gate e all'ownership cleanup S9. Lo staging resta evidence storica degli identifier production, ma il checkbox W8 viene riaperto finché il tree finale non è riprodotto in un nuovo disposable checkout.
- 2026-08-12: PR #14 viene mergiata come `c218e52bec331b3380e619cd2c6bfff8ae1e7b31`. Quel merge exact viene riprodotto senza patch in staging disposable; Application e Installer usano Keychain temporanee separate, Apple Notary Service restituisce `Accepted`, stapling e Gatekeeper passano, il checkout viene rimosso prima del preflight da cwd `/`, e il package prodotto è `94382d030a09e796b06d8cc8d79403b98f5cc0d384ca6c6366f4afea49b72366`.
- 2026-08-12: CHECKPOINT C falsifica il package `94382d03…`. `installer` restituisce successo e scrive receipt `com.maroffo.JidokaCode.pkg`, ma PackageKit risolve il bundle production già registrato nel worktree e logga `Applications/Jidoka Code.app relocated to .../build/Jidoka Code.app`; `/Applications/Jidoka Code.app` resta assente. Il package conteneva un `/pkg-info/relocate/bundle` nonostante `relocatable="false"`, e caller `umask 077` aveva prodotto directory/Mach-O `0700`. Il digest viene revocato; receipt e bundle rilocato restano failure evidence finché il cleanup non è autorizzato. La fix usa component policy non-relocatable, audit XPath e mode distributivi espliciti, poi richiede nuovo package/notarization/CHECKPOINT C.

## Outcomes and retrospective

W0, W1 S1-S9, W2-W7 e H1-H5 sono eseguiti. Ad-hoc è definitivamente scartato per lifecycle/install e resta soltanto fixture S1 esplicita; Developer ID Application e Installer, bounded signing e Keychain separate sono verificati. Il helper resta l'unica topologia che passa tutti i threshold ed è locked dalla decisione #53. Checkpoint B è accettato e il budget provider resta esaurito esattamente a 19/19 senza retry. Durable core, broker/reconciliation GitHub, transport Git app-managed, workflow Pi W5, coordinator W6, UI/lifecycle W7 e runtime Herdr visibile H sono verificati localmente. Herdr `0.8.0` è prerequisito esterno attestato e dispatch fallisce chiuso su drift. Il package notarizzato `94382d03…` è revocato dopo il falsifier installato di relocation/mode; `c73d854…` resta evidence storica. W8-W9 restano aperti per fix packaging, nuovo staging/notarization, cleanup autorizzato, nuovo CHECKPOINT C, installed lifecycle, runtime VoiceOver, credential GitHub/provider e default-session canary.
