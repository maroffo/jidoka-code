...
, onboarding_complete INTEGER NOT NULL DEFAULT 0
  CHECK (onboarding_complete IN (0, 1)), external_automation_acknowledged INTEGER NOT NULL DEFAULT 0
  CHECK (external_automation_acknowledged IN (0, 1)), provider_disclosure_acknowledged INTEGER NOT NULL DEFAULT 0
  CHECK (provider_disclosure_acknowledged IN (0, 1)), github_account TEXT, github_author_id INTEGER
  CHECK (github_author_id IS NULL OR github_author_id > 0), pending_github_account TEXT, pending_github_author_id INTEGER
  CHECK (pending_github_author_id IS NULL OR pending_github_author_id > 0), pending_replacement_sha256 TEXT
  CHECK (
  ...
  ), previous_github_account TEXT, credential_deletion_pending INTEGER NOT NULL DEFAULT 0
  CHECK (credential_deletion_pending IN (0, 1)), login_item_selected INTEGER NOT NULL DEFAULT 0
  CHECK (login_item_selected IN (0, 1)), login_item_status TEXT NOT NULL DEFAULT 'notRegistered'
  CHECK (login_item_status IN ('notRegistered', 'enabled', 'requiresApproval', 'notFound'))) STRICT;
CREATE TABLE jobs (
  id TEXT PRIMARY KEY,
...
BEGIN
  SELECT RAISE(ABORT, 'schema_migrations is append-only');
END;
CREATE TRIGGER job_steps_no_update
BEFORE UPDATE ON job_steps
BEGIN
  SELECT RAISE(ABORT, 'job_steps is append-only');
END;
CREATE TRIGGER job_steps_no_delete
BEFORE DELETE ON job_steps
BEGIN
  SELECT RAISE(ABORT, 'job_steps is append-only');
END;
CREATE TRIGGER job_transitions_no_update
BEFORE UPDATE ON job_transitions
BEGIN
  SELECT RAISE(ABORT, 'job_transitions is append-only');
END;
CREATE TRIGGER job_transitions_no_delete
...
BEGIN
  SELECT RAISE(ABORT, 'reconciliation_events is append-only');
END;
...
CREATE TABLE pi_run_launches (
  ...
  created_at REAL NOT NULL,
  updated_at REAL NOT NULL, execution_role_host_id TEXT REFERENCES herdr_replacement_role_hosts(id) ON DELETE RESTRICT,
...
END;
CREATE TRIGGER pi_run_session_origins_no_update
BEFORE UPDATE ON pi_run_session_origins
BEGIN
  SELECT RAISE(ABORT, 'pi_run_session_origins is append-only');
END;
CREATE TRIGGER pi_run_session_origins_no_delete
BEFORE DELETE ON pi_run_session_origins
BEGIN
  SELECT RAISE(ABORT, 'pi_run_session_origins is append-only');
END;
CREATE TRIGGER pi_run_results_no_update
BEFORE UPDATE ON pi_run_results
BEGIN
  SELECT RAISE(ABORT, 'pi_run_results is append-only');
END;
CREATE TRIGGER pi_run_results_no_delete
BEFORE DELETE ON pi_run_results
BEGIN
  SELECT RAISE(ABORT, 'pi_run_results is append-only');
END;
CREATE TRIGGER pi_run_events_no_update
BEFORE UPDATE ON pi_run_events
BEGIN
  SELECT RAISE(ABORT, 'pi_run_events is append-only');
END;
CREATE TRIGGER pi_run_events_no_delete
BEFORE DELETE ON pi_run_events
BEGIN
  SELECT RAISE(ABORT, 'pi_run_events is append-only');
END;
...
END;
CREATE TRIGGER approved_command_results_no_update
BEFORE UPDATE ON approved_command_results
BEGIN
  SELECT RAISE(ABORT, 'approved_command_results is append-only');
END;
CREATE TRIGGER approved_command_results_no_delete
BEFORE DELETE ON approved_command_results
BEGIN
  SELECT RAISE(ABORT, 'approved_command_results is append-only');
END;
CREATE TRIGGER approved_command_events_no_update
BEFORE UPDATE ON approved_command_events
BEGIN
  SELECT RAISE(ABORT, 'approved_command_events is append-only');
END;
CREATE TRIGGER approved_command_events_no_delete
BEFORE DELETE ON approved_command_events
BEGIN
  SELECT RAISE(ABORT, 'approved_command_events is append-only');
END;
...
CREATE TRIGGER herdr_repository_binding_history_no_update
BEFORE UPDATE ON herdr_repository_binding_history
BEGIN
  SELECT RAISE(ABORT, 'herdr_repository_binding_history is append-only');
END;
CREATE TRIGGER herdr_repository_binding_history_no_delete
BEFORE DELETE ON herdr_repository_binding_history
BEGIN
  SELECT RAISE(ABORT, 'herdr_repository_binding_history is append-only');
END;
  ...
  AND NEW.queue_sequence = host.last_queue_sequence + 1
    AND (
      ...
      OR (
        ...
        AND EXISTS (
          SELECT 1
          FROM herdr_q4_authority_candidates AS candidate
          JOIN herdr_topology_intents AS intent
            ON intent.kind = candidate.intent_kind
            AND intent.repository_id = candidate.repository_id
            AND intent.job_id = candidate.job_id
            AND intent.generation = candidate.generation
            AND intent.socket_device = candidate.socket_device
            ...
            AND EXISTS (
              ...
              WHERE prime_event.job_id = run.job_id
                ...
                AND prime_event.event_key GLOB (
                  'canary:*:' || candidate.event_kind || ':' || run.id || ':' ||
...
END;
INSERT INTO schema_migrations(version, name, applied_at) VALUES (1, 'durable-core', 1788774560.1929531);
INSERT INTO schema_migrations(version, name, applied_at) VALUES (2, 'application-ui-settings', 1788774560.1944261);
INSERT INTO schema_migrations(version, name, applied_at) VALUES (3, 'herdr-durable-runtime', 1788774560.1969471);
INSERT INTO schema_migrations(version, name, applied_at) VALUES (4, 'durable-approved-command-runs', 1788774560.1974459);
INSERT INTO schema_migrations(version, name, applied_at) VALUES (5, 'authorized-pre-session-pi-retry', 1788774560.197608);
INSERT INTO schema_migrations(version, name, applied_at) VALUES (6, 'authorized-pre-child-herdr-transaction-retry', 1788774560.197767);
INSERT INTO schema_migrations(version, name, applied_at) VALUES (7, 'authorized-legacy-agent-prime-retry', 1788774560.199635);
INSERT INTO schema_migrations(version, name, applied_at) VALUES (8, 'authorized-pane-agent-authority-reset', 1788774560.201577);
INSERT INTO schema_migrations(version, name, applied_at) VALUES (9, 'authorized-architecture-role-host-replacement-and-generation-rollover', 1788774560.209079);
INSERT INTO model_profiles(role, provider, model, thinking, updated_at) VALUES ('review', 'openai-codex', 'gpt-5.6-sol', 'max', 1788774560.194);
INSERT INTO model_profiles(role, provider, model, thinking, updated_at) VALUES ('triage', 'openai-codex', 'gpt-5.6-sol', 'max', 1788774560.194);
INSERT INTO model_profiles(role, provider, model, thinking, updated_at) VALUES ('planning', 'openai-codex', 'gpt-5.6-sol', 'max', 1788774560.194);
INSERT INTO model_profiles(role, provider, model, thinking, updated_at) VALUES ('orchestration', 'openai-codex', 'gpt-5.6-sol', 'max', 1788774560.194);
INSERT INTO app_settings(singleton, max_concurrency, paused, updated_at, onboarding_complete, external_automation_acknowledged, provider_disclosure_acknowledged, github_account, github_author_id, pending_github_account, pending_github_author_id, pending_replacement_sha256, previous_github_account, credential_deletion_pending, login_item_selected, login_item_status) VALUES (1, 2, 0, 1788774560.192, 0, 0, 0, NULL, NULL, NULL, NULL, NULL, NULL, 0, 0, 'notRegistered');
